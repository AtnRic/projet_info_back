...
export class AuthService {
    ...
    async register(createUserDto: CreateUserDto) {
        createUserDto.password = await bcrypt.hash(createUserDto.password, constants.salt);
    ...
    private async generateTokens(user) {
        ...
        return {
            access: await this.jwt.signAsync({exp: Math.floor(Date.now() / 1000) + 600,data: payload}),
            verify: await this.jwt.signAsync({exp: Math.floor(Date.now() / 1000) + (7*24*60*60), data: payload})
        }
    }
}
